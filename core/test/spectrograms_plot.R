...
bounded_frames <- dir("data")[dir("data") |> str_detect("bounded-frames")]
I <- dir("data")[dir("data") |> str_detect("I")]
...
for (i in 1:length(I)) {
  ...
  ggsave(paste0(rec, ".jpeg"), width = 30, height = 20, units = "cm")
}

I = read_csv("../../I.csv", col_names = FALSE)
#View(I)
bound_freqs = read_csv("../../bound_freqs.csv", col_names = FALSE)
...
I |>
  ...
  ggplot() +
  geom_tile(aes(frame, freq_bin, fill = dB)) +
  geom_point(data = bound_freqs |> mutate(frame = rownames(bound_freqs) |> as.integer()),
             aes(x = frame, y = 1, color = as_factor(X1)),
             size = .5) +
  scale_fill_continuous(high = "gray20", low = "gray90") +
  scale_color_manual(values = c("0" = "red", "1" = "green")) +
  xlim(c(300, 650))
